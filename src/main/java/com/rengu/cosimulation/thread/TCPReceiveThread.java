...
public class TCPReceiveThread {
    ...
    public void TCPMessageReceiver() {
        try {
            ...
            ServerSocket serverSocket = new ServerSocket(ApplicationConfig.TCP_RECEIVE_PORT);//创建了一个Socket类的实例，你可以使用它来发送和接受字节流
            while (true) {
    ...
    public void socketHandler(Socket socket) throws IOException {
        try {
            InputStream inputStream = socket.getInputStream();      //从客户端发送给服务器端的数据流。
            ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();//创建一个缓冲区。
            IOUtils.copy(inputStream, byteArrayOutputStream);
            bytesHandler(byteArrayOutputStream.toByteArray());//将字节流转为转为输出流
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
