...
public class ProjectService {
    ...
    public Project saveProject(Project project, String creatorId, String picId){
        ...
        if(!userService.hasUserById(creatorId)){                    // 根据Id查询用户是否存在
            throw new ResultException(ResultCode.PROJECT_CREATOR_ARGS_NOT_FOUND_ERROR);
        }
        project.setCreator(userService.getUserById(creatorId));     // 根据id查询用户
        ...
        if(hasProjectByNameAndDeleted(project.getName(), false)){       // 根据名称查询等项目是否存在
            throw new ResultException(ResultCode.PROJECT_NAME_EXISTED_ERROR); //该项目名已存在
        }
        if(!userService.hasUserById(picId)){
            throw new ResultException(ResultCode.PROJECT_PIC_ARGS_NOT_FOUND_ERROR); //请指定项目负责人
        }
    ...
    public boolean getProjectDetails(String projectId, String userId){
        ...
        if(ifOverTime(project.getFinishTime())){        // 节点计划
            project.setState(ApplicationConfig.PROJECT_OVER_TIME);          // 项目超时
        }
